...
import com.geesatcom.blog.bootstrap.ConsulYamlService;
 ...
 */
@Service
public class ScheduleTaskService implements SmartLifecycle {

    @Autowired
    @Qualifier(TestScheduleAutoConfiguration.CONFIG_WATCH_TASK_SCHEDULER_NAME2)
    ...
    public void start() {
        if (this.running.compareAndSet(false, true)) {
            this.watchFuture = this.taskScheduler.scheduleWithFixedDelay(this::watchConfigKeyValues2,
                    1000);
        }
    }
    ...
    @Timed("consul.watch-config-keys2")
    public void watchConfigKeyValues2() {
        if (!this.running.get()) {
            return;
        }
        System.out.println("循环任务执行。。。");
        String yaml = consulYamlService.getYaml(consulYamlService.getDefaultConsulKVPath());
        System.out.println("consul配置：\n" + yaml);
    }
